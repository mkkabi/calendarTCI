...
//    @Query(value = "select id, title, start_time, end_time, owner_id from events " +
//            "inner join event_attendees on id = event_id and attendee_id = ?1", nativeQuery = true)
//    List<Event> getAttendeesEvents(long userId);
...
//    @Query(value = "select id, title, start_time, end_time, day_of_week, week_number, owner_id " +
//            "from events where owner_id = ?1 and week_number = ?2 union " +
//            "select id, title, start_time, end_time, day_of_week, week_number, owner_id from events inner join event_attendees on id = event_id and " +
